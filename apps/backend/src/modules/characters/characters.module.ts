...
import { CharactersService } from './characters.service'
import { CreateCharacterDto, LockPreviewDto } from './dto/create-character.dto'
...
export class CharactersController {
  ...
  // POST /characters/:id/lock-preview — Character Lock Engine testen
  @Post(':id/lock-preview')
  lockPreview(@Param('id') id: string, @Body() dto: LockPreviewDto, @Request() req) {
    return this.service.lockPreview(id, dto, req.user.id)
  }
}
...
export class CharactersService {
  ...
  // Phase 3: Character Lock Engine direkt testen (ohne vollständigen Render-Job)
  async lockPreview(id: string, dto: LockPreviewDto, userId: string) {
    const character = await this.findOne(id, userId)

    const { data: result } = await firstValueFrom(
      this.http.post(`${this.aiWorkerUrl}/character-lock/apply`, {
        prompt: dto.prompt,
        negativePrompt: dto.negativePrompt || '',
        referenceImagePaths: character.referenceImages,
        method: character.profile ? 'auto' : 'ip_adapter',
        identityWeight: character.profile?.ipAdapterWeight || 0.8,
        hasFace: dto.hasFace ?? true,
    ...
    return {
      ...
      message: `🔒 Vorschau generiert mit ${result.methodUsed} (Identity Score: ${result.identityScore})`,
    }
  }
}
...
export class CreateCharacterDto {
  ...
  @ApiPropertyOptional({
    enum: ['auto', 'ip_adapter', 'pulid', 'instant_id', 'consis_id', 'photomaker_v2'],
    example: 'auto',
    description: 'Bevorzugte Character-Lock-Methode (Phase 3 Engine)',
  })
  @IsOptional()
  @IsString()
  preferredLockMethod?: string
}
...
export class LockPreviewDto {
  @ApiProperty({ example: 'Mika sitting in a sunny garden, smiling' })
  @IsString()
  prompt: string
